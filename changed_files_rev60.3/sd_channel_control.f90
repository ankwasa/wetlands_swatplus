      ...
      real :: depstt, depstmax, volrt, ttt
...
!!! written by Ann: calculate flow to wetlands and abstract this flow
      do ihr = 1, sp_ob%hru
        iobhr = hru(ihr)%obj_no
        ichan = ob(iobhr)%flood_ch_lnk
!        write(*,*) ihr, iobhr, ichan,  'ýes'
        if (ichan == jrch) then
          ires = hru(ihr)%dbs%surf_stor
          if (ires >0) then
            depstmax = wet_ob(ihr)%evol
            depstt = depstmax - wet(ihr)%flo
            depstt = max(0., depstt)
            ttt=1.
            if (const * ob(icmd)%hin%flo > depstt) then
            ttt=depstt /(const * ob(icmd)%hin%flo)
            end if
!      write(*,*) jrch, ires, ihr, iobhr, ichan
            if (depstt > 0.) then
!                wet(ihr)%flo = wet(ires)%flo + depst 
!                wet_in_d(ihr) =  ttt * const * ht1
!                wet(ihr) = wet(ihr) + wet_in_d(ihr)
!                ht1= (1.-ttt * const)*ht1
!                
!                volrt = volrt * (1.-ttt*const)
!                vol_ovb = vol_ovb * (1.-ttt*const)
!                vol = vol * (1.-ttt*const)
